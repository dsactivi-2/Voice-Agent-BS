...
describe('getNextPhase', () => {
  ...
  it('transitions hook -> qualify when interest > 0.3', () => {
    ...
    const result = getNextPhase(session, {
      interest_score: 0.35,
      reply_text: 'Zanimljivo, recite mi vise.',
    });
  ...
  it('transitions pitch -> objection when objection detected', () => {
    const session = createMockSession({ phase: 'pitch' });
    const result = getNextPhase(session, {
      interest_score: 0.6,
      reply_text: 'Ne zanima me to uopste.',
    });
    expect(result).toBe('objection');
  });

  it('transitions pitch -> close when interest > 0.72 and no objection', () => {
    const session = createMockSession({ phase: 'pitch' });
    const result = getNextPhase(session, {
      interest_score: 0.8,
      reply_text: 'To je zaista odlicna ponuda.',
    });
    expect(result).toBe('close');
  });

  it('prioritizes objection over close in pitch phase', () => {
    const session = createMockSession({ phase: 'pitch' });
    const result = getNextPhase(session, {
      interest_score: 0.9,
      reply_text: 'Ali ne mogu to priustiti, preskupo je.',
    });
    expect(result).toBe('objection');
  });

  it('stays in pitch when interest <= 0.72 and no objection', () => {
    const session = createMockSession({ phase: 'pitch' });
    const result = getNextPhase(session, {
      interest_score: 0.6,
      reply_text: 'Hmm, zanimljivo.',
    });
    expect(result).toBe('pitch');
  });
  ...
  it('transitions objection -> close when interest > 0.72', () => {
    ...
    const result = getNextPhase(session, {
      interest_score: 0.8,
      reply_text: 'Zapravo, to ima smisla.',
    });
    expect(result).toBe('close');
  });
  ...
  it('transitions objection -> pitch when interest <= 0.72', () => {
    ...
    const result = getNextPhase(session, {
      interest_score: 0.5,
      reply_text: 'Hmm, mozda.',
    });
    expect(result).toBe('pitch');
  });

  it('transitions close -> confirm when agreement detected', () => {
    const session = createMockSession({ phase: 'close' });
    const result = getNextPhase(session, {
      interest_score: 0.9,
      reply_text: 'Da, prihvatam ponudu.',
    });
    expect(result).toBe('confirm');
  });

  it('transitions close -> objection when objection detected', () => {
    const session = createMockSession({ phase: 'close' });
    const result = getNextPhase(session, {
      interest_score: 0.4,
      reply_text: 'Ne mogu to sebi priustiti, skupo je.',
    });
    expect(result).toBe('objection');
  });

  it('stays in close when neither agreement nor objection', () => {
    const session = createMockSession({ phase: 'close' });
    const result = getNextPhase(session, {
      interest_score: 0.6,
      reply_text: 'Razmislit cu o tome.',
    });
    expect(result).toBe('close');
  });
...
describe('hasObjection', () => {
  ...
  it('detects "ali" as an objection', () => {
    expect(hasObjection('Ali ja to ne razumijem')).toBe(true);
  });

  it('detects "problem" as an objection', () => {
    expect(hasObjection('Imam problem sa tim')).toBe(true);
  });
});
...
describe('hasAgreement', () => {
  ...
  it('detects "moze" as agreement', () => {
    expect(hasAgreement('Moze, u redu')).toBe(true);
  });
  ...
  it('detects "odlicno" as agreement', () => {
    expect(hasAgreement('Odlicno, to mi odgovara')).toBe(true);
  });

  it('detects "zelim" as agreement', () => {
    expect(hasAgreement('Zelim to aktivirati')).toBe(true);
  });
});
