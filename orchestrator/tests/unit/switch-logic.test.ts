...
describe('shouldSwitchToFull', () => {
  ...
  it('returns true when interest average exceeds threshold in pitch phase', () => {
    const session = createMockSession({
      phase: 'pitch',
      interestScores: [0.8, 0.85, 0.9],
    });
    expect(shouldSwitchToFull(session)).toBe(true);
  });

  it('returns true when complexity exceeds threshold in close phase', () => {
    const session = createMockSession({
      phase: 'close',
      complexityScore: 0.75,
      interestScores: [0.3, 0.3, 0.3],
    });
    expect(shouldSwitchToFull(session)).toBe(true);
  });
  ...
  it('returns true in objection phase with high complexity', () => {
    const session = createMockSession({
      phase: 'objection',
      complexityScore: 0.65,
      interestScores: [0.4, 0.4, 0.4],
    });
    expect(shouldSwitchToFull(session)).toBe(true);
  });
});
